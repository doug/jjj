#!/usr/bin/env bash
#
# Seed the synthetic decomposition target.
#
...
git add -A && git commit -q -m "seed: synthetic pipeline"
...
"$JJJ" problem new "The pipeline costs 700,004 operations; make it cost far fewer" \
...
\`./score.sh\` shows where the operations go. Read it before conjecturing: one
of the functions in here is long, ugly, and costs almost nothing." >/dev/null

cat > "$ROOT/SWARM.md" <<'BRIEF'
# The target

`fixture/pipeline.py` costs 700,004 operations for a fixed workload. Make it
cost far fewer, without changing the answer.

## The score

`./score.sh` prints `<score> 100` and shows where the operations go. Baseline is
13; the scale is logarithmic down to 60,000 operations, so it keeps paying all
the way down rather than saturating once the obvious win is taken.
...
- **Say what matters.** `jjj rank set <problem>...` and `jjj rank move`. A
  ranking is a judgement about what will matter to what comes after — what
  unblocks other work, what is foundational — not a re-statement of the biggest
  number in the profile.
- **Spread out.** In an earlier trial nobody ranked anything, six agents
  converged on one problem while others sat untouched, and 62% of solutions were
  withdrawn as superseded. That waste was concentration, not competition.
